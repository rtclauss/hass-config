...
def test_every_behavior_and_helper_has_explicit_labels() -> None:
    ...
    assert sum(item["source"] == "repository" for item in manifest["behaviors"]) == 407
...
def test_new_presence_emitters_and_tesla_cancel_have_domain_labels() -> None:
    by_unique_id = {
        item.get("unique_id"): set(item["labels"])
        for item in _manifest()["behaviors"]
        if item["source"] == "repository"
    }

    assert by_unique_id["tesla_departure_cancel_when_home_context_ends"] == {
        "arrival_scope",
        "presence_house_mode",
        "vehicles_ev",
    }
    for unique_id in ("zeke_arrival_emitter", "zeke_departure_emitter"):
        assert by_unique_id[unique_id] == {
            "arrival_scope",
            "presence_house_mode",
        }
