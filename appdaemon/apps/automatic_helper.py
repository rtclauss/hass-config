...
class AutomaticHelper(hass.Hass):
    ...
    def event_received(self, event_name, data, kwargs):
        ...
        now = datetime.now()
        driving_time = datetime.now() - timedelta(minutes=self.driving_time)
        ...
        if event_type == "ignition:on":
            if location == "Work":
                if not self.now_is_between(driving_time, calendar_start_time):
                    self.log("Looks like you're not curling right away. Turning on Nest")
